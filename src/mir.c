...
struct MIR_context {
  ...
  /* Frame base published by generated code right before a GC safepoint call.  Generated
     code stores its canonical frame base (frame pointer) here, as there is no portable
     way for a C callee to find its caller's frame pointer (e.g. __builtin_frame_address
     (1) is unsupported on Win64).  */
  void *gc_frame_base;
...
/* Address of the cell into which generated code publishes its canonical frame base
   right before a GC safepoint call.  The generator bakes this address into the code.  */
void **_MIR_gc_frame_base_addr (MIR_context_t ctx) { return &ctx->gc_frame_base; }
...
size_t MIR_visit_gc_roots (MIR_context_t ctx, MIR_item_t func_item, void *return_pc,
  ...
  /* NULL frame_base means "use the frame base the generated code published at the
     safepoint" -- the portable way, see gc_frame_base in struct MIR_context.  */
  if (frame_base == NULL) frame_base = ctx->gc_frame_base;
