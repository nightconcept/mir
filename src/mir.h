...
/* Visit the GC roots live at the safepoint whose return address is RETURN_PC.  FRAME_BASE
   should normally be NULL: generated code publishes its canonical frame base right before
   every safepoint call, and NULL means "use that".  Pass a non-NULL FRAME_BASE only if you
   can compute the MIR frame base yourself -- note that __builtin_frame_address (1) can not
   do it (it is unsupported for depth > 0 on Win64 and needs an unbroken frame-pointer
   chain elsewhere).  */
...
extern void **_MIR_gc_frame_base_addr (MIR_context_t ctx);
